...
kyznechick_asm_152:
    ...
    mov r10, LS_mat         ; r11 <- LS_mat
    lea rdx, [r10 + 01000h]   ; rdx <- LS_mat[1]
    lea r11, [rdx + 01000h]   ; r11 <- LS_mat[2]
    ; Основной цикл раундов
    pxor xmm0, [rcx]         ; xmm0 <- in ^ round_keys[i]
    add rcx, 16              ; Переход к следующему раундовому ключу
    ...
    pxor xmm0, [r11+rax+01000h]         ; xmm0 <- xmm0 ^ LS_mat[3][in_out[3]]

    ; Байт 4 (чётный)
    pextrw eax, xmm2, 2      ; eax <- in_out[4] * 16
    pxor xmm0, [r11+rax+02000h]         ; xmm0 <- xmm0 ^ LS_mat[4][in_out[4]]

    ; Байт 5 (нечётный)
    pextrw eax, xmm3, 2      ; eax <- in_out[5] * 16
    pxor xmm0, [r11+rax+03000h]         ; xmm0 <- xmm0 ^ LS_mat[5][in_out[5]]

    ; Байт 6 (чётный)
    pextrw eax, xmm2, 3      ; eax <- in_out[6] * 16
    pxor xmm0, [r11+rax+04000h]         ; xmm0 <- xmm0 ^ LS_mat[6][in_out[6]]

    ; Байт 7 (нечётный)
    pextrw eax, xmm3, 3      ; eax <- in_out[7] * 16
    pxor xmm0, [r11+rax+05000h]         ; xmm0 <- xmm0 ^ LS_mat[7][in_out[7]]

    ; Байт 8 (чётный)
    pextrw eax, xmm2, 4      ; eax <- in_out[8] * 16
    pxor xmm0, [r11+rax+06000h]         ; xmm0 <- xmm0 ^ LS_mat[8][in_out[8]]

    ; Байт 9 (нечётный)
    pextrw eax, xmm3, 4      ; eax <- in_out[9] * 16
    pxor xmm0, [r11+rax+07000h]         ; xmm0 <- xmm0 ^ LS_mat[9][in_out[9]]

    ; Байт 10 (чётный)
    pextrw eax, xmm2, 5      ; eax <- in_out[10] * 16
    pxor xmm0, [r11+rax+08000h]         ; xmm0 <- xmm0 ^ LS_mat[10][in_out[10]]

    ; Байт 11 (нечётный)
    pextrw eax, xmm3, 5      ; eax <- in_out[11] * 16
    pxor xmm0, [r11+rax+09000h]         ; xmm0 <- xmm0 ^ LS_mat[11][in_out[11]]

    ; Байт 12 (чётный)
    pextrw eax, xmm2, 6      ; eax <- in_out[12] * 16
    pxor xmm0, [r11+rax+0A000h]         ; xmm0 <- xmm0 ^ LS_mat[12][in_out[12]]

    ; Байт 13 (нечётный)
    pextrw eax, xmm3, 6      ; eax <- in_out[13] * 16
    pxor xmm0, [r11+rax+0B000h]         ; xmm0 <- xmm0 ^ LS_mat[13][in_out[13]]

    ; Байт 14 (чётный)
    pextrw eax, xmm2, 7      ; eax <- in_out[14] * 16
    pxor xmm0, [r11+rax+0C000h]         ; xmm0 <- xmm0 ^ LS_mat[14][in_out[14]]

    ; Байт 15 (нечётный)
    pextrw eax, xmm3, 7      ; eax <- in_out[15] * 16
    pxor xmm0, [r11+rax+0D000h]         ; xmm0 <- xmm0 ^ LS_mat[15][in_out[15]]
